...
function requestBinder(streamer, request) {
	...
	request.on('timeout', timeoutHandler.bind(streamer));
	if(streamer.options.timeout) {
		request.setTimeout(streamer.options.timeout);
	}
}
...
function Streamer(request, options) {
